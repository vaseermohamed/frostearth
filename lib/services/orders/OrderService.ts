...
export class OrderService {
   ...
   */
  async createPendingOrder(productIds: string[], buyerEmail: string, buyerPhone?: string) {
  ...
  private async markPaidByRazorpayOrderId(
    ...
    if (updated.status === "PAID") {
      ...
      await this.sendReceiptEmail(updated.buyerEmail, tokens);
    }
   ...
   */
  private async sendReceiptEmail(to: string, tokens: { title: string; token: string }[]) {
    ...
    try {
      await this.email.send({
        to,
        subject: "Your FrostEarth download links",
        html: `<p>Thanks for your purchase! Here ${tokens.length > 1 ? "are your files" : "is your file"}:</p><ul>${linksHtml}</ul><p>These links expire in 3 days.</p>`,
      });
    } catch (err) {
